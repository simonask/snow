...
TEST_CASE(assignments) {
	...
	TEST_EQ(snow_value_to_int(v), correct);
}
...
TEST_CASE(named_arguments) {
	...
	for (uintx i = 0; i < 5; ++i)
	{
		TEST(snow_is_object(r[i]));
		SnArray* ar = (SnArray*)r[i];
		TEST(snow_typeof(ar) == SnArrayType);
		TEST(snow_array_get(ar, 0) == snow_int_to_value(1));
		TEST(snow_array_get(ar, 1) == snow_int_to_value(2));
	}
}
...
/*TEST_CASE(string_interpolation) {
	...
	TEST_EQ(snow_value_to_cstr(v), correct);
