...
static create_ast_node(SnAstNodeType type, ...)
{
	SnAstNode* node = (SnAstNode*)snow_alloc_any_object(sizeof(SnAstNode) + ast_size[type]*sizeof(VALUE));
	node->type = type;
...
SnAstNode* snow_ast_function(const char* name, const char* file, SnAstNode* seq_args, SnAstNode* seq_def) {
	...
	return create_ast_node(snow_create_string(name), snow_create_string(file), seq_args, seq_def);
}
